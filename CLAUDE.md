...
9. **Signal generation** — Three sub-stages (Phase 1 of the regime-router refactor):
   - **Regime classification** — `regime` string (`SQUEEZE` / `BREAKOUT` / `EXHAUSTED` / `TREND` / `RANGE`) derived from `is_squeezing`, `sqz_release`, `stretch_factor`, `adx`. **Descriptive only** — it gates nothing yet.
   - **Strategy modules** — exactly one is registered: **Trend Pullback** (`tp_long`/`tp_short`/`tp_quality`), which is the anchor + 5-component confluence + `passes_quality_filter` logic, condition-for-condition unchanged from pre-refactor. `sigTime` ("Trend" fires off `trendUp`/`trendDown`; "Score" fires off `is_bull`/`is_bear` + `entry_is_fresh` + explicit `barstate.isconfirmed`) is a **mode of this module**. Both modes AND in `in_session` and `anchor_ready`.
   - **Regime router** (stub) — always selects Trend Pullback: `active_strategy = "Trend Pullback"`, `strat_long/strat_short/strat_quality` alias the `tp_*` outputs. Phase 2+ registers sibling modules (ORB, VWAP fade) and gives this real selection.
   - **Signal dispatch** — `signal_buy = strat_long and not b_fired`, `signal_sell = strat_short and not s_fired`. `b_fired`/`s_fired` enforce strict BUY/SELL alternation across any number of regime/trend flips; they clear only when the opposite signal fires or at RTH session open (`useSessionFilter`-gated) — **not** on a trend-direction change by itself.
   Signals are non-repainting; `barstate.isconfirmed` guards every flip condition (Trend mode via `trendUp`/`trendDown` already carrying it, Score mode explicitly). RTH filter via `time(timeframe.period, "0930-1600:23456")` — `in_session`/`session_just_opened` are computed in their own **RTH Session Window** section placed *before* P&L exit tracking, because the session-open reset must flatten stale position state before `current_pnl` is evaluated on that same bar. Score-mode signals can also be constrained to fire only within `maxBarsFromFlip` bars of the anchor flip (default 0 = disabled); Trend mode is unaffected.
...
11. **Success rate tracking** — Rolling arrays (`buy_results`/`sell_results`), capped at `maxSignalsToTrack`; every signal is scored won/lost when the next opposite-direction signal closes it — won only if the target was reached during the entry, lost otherwise.
12. **Visuals** — Conditional plots, one branch per anchor: EMA9 dynamic line (green/red), or SMA line (green/red) plus its grey ATR buffer band; flip circles at transition bars; BUY/SELL labels anchored to `anchor_line_price`.
13. **Dashboard** — `table.new` at `position.bottom_right` with `DASHBOARD_MAX_ROWS = 28`; rendered only on `barstate.islast`. The table is `var`, so every render starts with `table.clear(d, 0, 0, 1, DASHBOARD_MAX_ROWS - 1)` — without it, rows written by the variable-length gate-detail loop persist after a gate deactivates and keep showing a stale warning. The gate detail loop is the last section written; it has an explicit `if row >= DASHBOARD_MAX_ROWS: break` guard because it is the only variable-length section. All preceding rows are bounded by design. The first data row reports the **active anchor** (`Trend (EMA9)` or `Trend (SMA)`) and its own price. Both that row and the four BUY/SELL label sites read the shared `anchor_line_price`/`anchor_short` globals rather than each re-deriving the anchor. The **Strategy** row (formerly "Market State") shows `active_strategy · <regime>` — Phase 1: always `Trend Pullback`, with the regime word purely descriptive. Row count is unchanged by the refactor. `dashboard_score` reads `strat_quality` (== `final_score` while Trend Pullback is the only module).
...
**Strategy router (phased refactor, currently Phase 1):** Signal generation is structured as *regime classification → strategy modules → router → dispatch*. Only one module exists — **Trend Pullback** (`tp_long`/`tp_short`/`tp_quality`), the anchor + confluence logic verbatim — and the router (`active_strategy`/`strat_long`/`strat_short`/`strat_quality`) unconditionally selects it, so signals are identical to the pre-refactor script. `regime` is computed but gates nothing. The point of Phase 1 is that every downstream consumer (P&L, win-rate, labels, alerts, dashboard) now fires off the dispatched `signal_buy`/`signal_sell` / the `strat_*` surface, so Phase 2 can add sibling modules (ORB, VWAP fade, HOD/LOD break, prior-day levels) and real regime-based routing without touching those consumers. When adding a module: expose `<name>_long`/`<name>_short`/`<name>_quality`, register it in the router, and let dispatch apply session/warm-up/alternation — do not re-implement those per module.
...
18. **Strategy router (Phase 1) — no behavior change:** the **Strategy** dashboard row reads `Trend Pullback · <regime>`; signals fire on exactly the same bars as before the refactor (spot-check a handful of BUY/SELL bars against a pre-refactor build in both `sigTime` modes); dashboard row count is unchanged (Strategy replaces the old Market State row, not an addition)
