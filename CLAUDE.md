...
**Gate change — velocity override extended to Gate 2:** `is_high_velocity` (`ADX > 35` rising 3 bars) now waives the Stretch penalty as well as the ATR-Fuel penalty. A strong, still-accelerating trend that has run past `ext_scale` ATR keeps its `final_score` intact instead of being docked −10/−20 and pushed below the star filter. The raw stretch tier is still shown as advisory (`gate2_raw`, gate message reads `🔥 VELOCITY` in place of `-N`); only the score subtraction, `active_gate_count`, and the `EXHAUSTED` regime label are suppressed. Implemented as `gate2_stretched = gate2_raw and not is_high_velocity`, mirroring the pre-existing `gate3_fuel` pattern.
