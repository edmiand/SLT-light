...
11. **Success rate tracking** — Rolling arrays (`buy_results`/`sell_results`), capped at `maxSignalsToTrack`; a signal is scored won/lost when the next opposite-direction signal closes it, but ONLY if it actually resolved — won if PROFIT (`target_reached`) fired during the entry, lost if LOSS (`stop_reached`) fired. An entry reversed before hitting either is **unresolved** and is excluded from the arrays entirely — it counts toward neither win nor loss, so the win-rate % is a clean wins/(wins+losses) ratio. `buy_signals_fired`/`sell_signals_fired` (`var int`) separately count every signal fired regardless of resolution, shown alongside the % for context but not part of it. **Phase 4:** the same result is *also* pushed into a per-module store (`mod_results`, a 3-slot `array<ModResults>` indexed by `strat_id()`), keyed by the module that **opened** the entry (`buy_entry_strategy`/`sell_entry_strategy`, stamped from `active_strategy` at open). Overall arrays stay module-blind. The dashboard shows the per-module breakdown only in a multi-module build; the light build (one live module) shows just the overall BUY/SELL rates.
...
**Pinned constants** (`FIXED BEHAVIOUR` block, top of file): `showLabels=true`, `sigTime="Score"`, `gateOn=true`, `rvolMode="Time-of-Day"`, `rvolLookbackDays=10`, `enableSuccessRate=true`, `maxSignalsToTrack=50`, `showSignalPnL=true`, `showCircles=true`, `showBg=true`. (`showDash` / `extDash` are inputs in `grp_vis`; `tradingHours` is an input in `grp_hours`.)
...
**Win-rate tracking — only resolved entries count, unresolved reversals excluded:** `buy_results`/`sell_results` are pushed to *only if an entry actually resolved* — hit PROFIT (win, `true`) or LOSS (loss, `false`) — during the time it was open. An entry's outcome isn't known until it's closed by the next opposite-direction signal (per strict alternation, there's exactly one open position per direction at a time), so `buy_won_this_entry`/`sell_won_this_entry`/`buy_lost_this_entry`/`sell_lost_this_entry` (`var bool`) accumulate whether `target_reached`/`stop_reached` fired at any point during the entry — at most one of the won/lost pair per entry, since `pnl_exit_fired` latches after the first of the two fires. When the opposite signal fires: **if** either flag is set, that outcome is pushed into the results array and reset for the new entry; **if neither** fired (the position was simply reversed — trend flipped back before price ever reached ±`pnlTarget`), nothing is pushed — that signal counts toward neither win nor loss, so `success_rate_buy`/`success_rate_sell` stay a clean wins/(wins+losses) ratio, not diluted by reversed-but-never-resolved entries. `buy_signals_fired`/`sell_signals_fired` (`var int`, incremented at open, never reset or capped) separately track every signal fired regardless of resolution — shown on the dashboard as `(N fired)` for context (how often signals get reversed before resolving) but excluded from the % calculation itself. `buy_entry_open`/`sell_entry_open` (`var bool`) guard against scoring before any entry has actually opened. Each results array is an independent rolling `array<bool>` capped at `maxSignalsToTrack` via FIFO (`array.shift` on overflow) and holds *resolved entries only*, so its size is `wins + losses`, never including unresolved ones. `calc_win_rate(arr, enabled) => [wins, total, rate]` computes stats for every array from one shared read-only function — unchanged by this design, since it just reads whatever `true`/`false` values are actually in the array. The currently-open entry is never counted until it closes.

**Per-module win-rate (Phase 4):** every place that pushes into `buy_results`/`sell_results` — the two closes in *UPDATE SUCCESS RATE TRACKING* and the session-open carry-over close — also calls `record_module(mod_results, strat_id(<dir>_entry_strategy), is_buy, won, cap)`, under the same "only if resolved" guard, so `mod_results` follows the same unresolved-excluded semantics as the overall arrays. `mod_results` is a `var array<ModResults>` (UDT wrapping `array<bool> buys` / `array<bool> sells`, because Pine arrays can't nest — same trick as `TodSession`), populated once with 3 slots. `strat_id()` still maps `0` Trend Pullback / `1` ORB / `2` VWAP Fade (anything unknown → 0) — the ORB and Fade mappings are vestigial in the light build. `<dir>_entry_strategy` (`var string`) is stamped from `active_strategy` at the moment the entry opens. `record_module` mutates the UDT's inner array in place (reference type reachable through the `store` parameter). The per-module dashboard rows are gated on `multi_module` (`= orbEnabled`) — in the single-module light build they are suppressed entirely because they would duplicate the BUY/SELL Win Rate rows. `record_module` still runs, so slot 0 accumulates and the rows would render correctly the moment a second module is live; when shown they match the BUY/SELL Win Rate style (white label, value colour-coded by the module's combined rate via `calc_win_rate` on `mr.buys` + `mr.sells`).
...
2. **Settings panel shows only:** Signal Anchor, Score Stars, P&L Exit (Enable + Target %), Trading Hours (session picker), and Dashboard & Visuals (Show Dashboard + Extended Metrics). No SMA / Signal-timing / RVOL / ORB / Fade / Success groups; no flip-circle / background toggles.
...
13. **Win rate:** dashboard shows `BUY Win Rate` / `SELL Win Rate` as `<rate>% [<wins>/<resolved>] (<N> fired)`; a BUY that hits PROFIT then is closed by the next SELL counts as a win; hits LOSS → loss; **closed with neither PROFIT nor LOSS hit → excluded entirely** (counts toward neither win nor loss, but still increments the `(N fired)` count); the currently open signal is never counted. Confirm the `fired` count is always ≥ the resolved total (`[w/t]`), and that reversing a signal repeatedly without ever reaching `pnlTarget` grows `fired` without moving the win rate %. No per-module win-rate rows render in the light build (`multi_module` is false); `mod_results` slot 0 still accumulates in the background under the same resolved-only rule.
