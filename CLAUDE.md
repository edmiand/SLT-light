...
1. **Constants** — Gate penalty values, dashboard sizing (`DASHBOARD_MAX_ROWS = 30`)
...
9. **Signal generation** — Four sub-stages (regime-router refactor, **Phase 2**: two modules):
   - **Regime classification** — `regime` string (`SQUEEZE` / `BREAKOUT` / `EXHAUSTED` / `TREND` / `RANGE`) derived from `is_squeezing`, `sqz_release`, `stretch_factor`, `adx`. **Descriptive only** — feeds the dashboard, gates nothing.
   ...
   - **Regime router** — `active_strategy` / `strat_long` / `strat_short` / `strat_quality`. Default = Trend Pullback; an ORB breakout (`orb_long or orb_short`) **preempts** it on that bar. The router only chooses between modules — it never loosens a module's own gating.
   - **Signal dispatch** — `signal_buy = strat_long and not b_fired`, `signal_sell = strat_short and not s_fired`. `b_fired`/`s_fired` enforce strict BUY/SELL alternation **across both modules and any number of regime/trend flips**; they clear only when the opposite signal fires or at RTH session open (`useSessionFilter`-gated) — **not** on a trend-direction change by itself.
...
**Strategy router (phased refactor, currently Phase 2):** Signal generation is structured as *regime classification → strategy modules → router → dispatch*. Two modules exist: **Trend Pullback** (`tp_*`, the anchor + confluence logic verbatim) and **ORB** (`orb_*`, see [ORB Module](#orb-module)). The router (`active_strategy`/`strat_long`/`strat_short`/`strat_quality`) defaults to Trend Pullback and lets an ORB breakout preempt it for that bar. `regime` is computed but gates nothing yet. Every downstream consumer (P&L, win-rate, labels, alerts, dashboard) fires off the dispatched `signal_buy`/`signal_sell` / the `strat_*` surface, so modules are added without touching them. **When adding a module:** expose `<name>_long`/`<name>_short`/`<name>_quality`, keep it fully self-gated, register it in the router, and let dispatch apply alternation — do not re-implement session/alternation per module. Still to come: Phase 3 (VWAP fade, HOD/LOD break, prior-day levels), Phase 4 (per-module win-rate via generalized results arrays; `regime` becoming a real eligibility gate).
...
**Dashboard row budget:** `DASHBOARD_MAX_ROWS = 30` (rows 0–29), bumped from 28 in Phase 2. The optional **ORB** row (rendered only when `orbEnabled`) is the one new fixed row; the +2 cap keeps the same ~1 row of headroom at max config (all options + 4 gates + Extended Metrics + Success Rate + ORB). The gate detail loop is the last section; it has an explicit `if row >= DASHBOARD_MAX_ROWS: break` guard because it is the only variable-length section (all other rows are fixed-count by design). Success Rate Tracking occupies exactly 3 rows (separator + BUY Win Rate + SELL Win Rate) whenever enabled, regardless of Extended Metrics.
...
8. **Dashboard row count:** with Success Rate Tracking + all 4 gates active + Extended Metrics ON + ORB module enabled, confirm no runtime error (row overflow guard working; `DASHBOARD_MAX_ROWS = 30`)
...
17. **Entry freshness:** in Score mode, set `Max Bars From Flip = 5` → confirm no BUY/SELL fires on bar 6+ after a flip (watch the "Bars From Flip" Extended Metrics row to confirm the count itself resets to 0 on each flip bar); confirm Trend mode is unaffected by the same setting; confirm worst case (Success Rate + Extended Metrics + all 4 gates + ORB on) still shows no dashboard row overflow with `DASHBOARD_MAX_ROWS = 30`
18. **Strategy router (Phase 1) — no behavior change:** the **Strategy** dashboard row reads `Trend Pullback · <regime>`; with the **ORB module disabled** (`orbEnabled = false`), signals fire on exactly the same bars as before the refactor (spot-check a handful of BUY/SELL bars against a pre-refactor build in both `sigTime` modes); the ORB row is absent and row count matches the pre-Phase-2 dashboard.
